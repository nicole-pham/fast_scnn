...
num_epochs = 100 # changed epochs from 100 to 10 for time
batch_size = 64 # increased batch size (images are 256x256 instead of 6000x6000)
learning_rate = 1e-3
...
def preprocessing(image, mask):
    ...
    return image_transformer(image).float(), torch.tensor(mask, dtype=torch.float32)
...
ds_train = PotsdamDataset(train_image_path, train_label_path, transform=preprocessing)
ds_test = PotsdamDataset(test_image_path, test_label_path, transform=preprocessing)
...
fit_res = trainer.fit(dl_train,
                      ...
                      checkpoints='checkpoints/100' + model.__class__.__name__ + datetime.datetime.today().strftime("%m_%d"))
